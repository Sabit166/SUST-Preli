"""Local smoke test: run all 10 sample cases through the service in-process
and diff the exact-match fields against the expected outputs."""
...
# Inline 10 sample cases (input + expected_output) from the public pack.
CASES: list[dict] = [
    {
        "id": "SAMPLE-01",
...
def main() -> None:
    ...
    for case in CASES:
        ...
        if resp.status_code != 200:
            print(f"[FAIL] {cid}: HTTP {resp.status_code} body={resp.text[:200]}")
            continue
        actual = resp.json()
        ...
        for f in EXACT_FIELDS:
            if actual.get(f) != expected.get(f):
                mismatches.append(
                    f"  {f}: expected={expected.get(f)!r} actual={actual.get(f)!r}"
                )
        if mismatches:
            ...
            passed += 1
            print(f"[OK]   {cid} ({case['label']})")

    print(f"\n{passed}/{total} cases passed exact-match fields.")
